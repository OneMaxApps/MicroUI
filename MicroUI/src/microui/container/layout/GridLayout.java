...
//		this(0, 0, ctx.width, ctx.height, cells, cells);
//	}
//
//	public GridLayout(int columns, int rows) {
//		this(0, 0, ctx.width, ctx.height, columns, rows);
//	}
//
//	public GridLayout(float x, float y, float w, float h) {
//		this(x, y, w, h, 3, 3);
//	}
//
...
//					ctx.rect(x, y, getWidth() / cols, getHeight() / rows);
//				}
//			}
//		}
//		ctx.popStyle();
...
//		if (col < 0 || col > getColumns() - 1 || row < 0 || row > getRows() - 1) {
//			throw new IndexOutOfBoundsException("index out of bounds of grid");
//		}
//		
...
//			if (column == columnList.get(i) && row == rowList.get(i)) {
//				empty = false;
//			}
//		}
//
...
//		}
//
//		mainLoop: for (int row = 0; row < rows; row++) {
...
//			} else {
//				bounds.setSize(min(elementDefaultWidth.get(i), getWidth() / getColumns()),
//						min(elementDefaultHeight.get(i), getHeight() / getRows()));
//			}
//
//			if (bounds instanceof Container container) {
//				container.setX(map(columnList.get(i), 0, cols, getX(), getX() + getWidth())
//						+ ((getWidth() / getColumns() / 2) - container.getRealWidth() / 2));
//				container.setY(map(rowList.get(i), 0, rows, getY(), getY() + getHeight())
//						+ ((getHeight() / getRows() / 2) - container.getRealHeight() / 2));
//
//			} else {
//				if (bounds instanceof Diall dial) {
//					dial.setPosition(map(columnList.get(i), 0, GridLayout.this.cols, getX(), getX() + getWidth())
//							+ ((getWidth() / getColumns()) / 2) - (Math.min(dial.getWidth(), dial.getHeight())) / 2,
//							map(rowList.get(i), 0, GridLayout.this.rows, getY(), getY() + getHeight())
//									+ ((getHeight() / getRows()) / 2)
//									- (Math.min(dial.getWidth(), dial.getHeight())) / 2);
//				} else {
//					bounds.setPosition(
//							map(columnList.get(i), 0, GridLayout.this.cols, getX(), getX() + getWidth())
//									+ ((getWidth() / getColumns()) / 2) - bounds.getWidth() / 2,
//							map(rowList.get(i), 0, GridLayout.this.rows, getY(), getY() + getHeight())
//									+ ((getHeight() / getRows()) / 2) - bounds.getHeight() / 2);
//				}
//			}
//
//		}
