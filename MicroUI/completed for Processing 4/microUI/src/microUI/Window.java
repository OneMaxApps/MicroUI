...
public class Window extends Rectangle {
	...
	public void draw() {
		...
		if(resizable) {
			...
			if(canResize) {
				setSize(getW()+app.mouseX-app.pmouseX,getH()+app.mouseY-app.pmouseY);
			}
		}
	...
	public final void setLayout(Layout layout) {
		this.layout = layout;
		if(bar.isVisible) {
			layout.setPosition(x, y+bar.layout.getH());
			layout.setSize(w, h-bar.layout.getH());
		} else {
	...
	public final class Bar {
		...
		public Bar(PApplet app, String title) {
			layout = new RowLayout(app,x,y,w,app.height*.08f);
			layout.add("", .1f);
		...
		public final void draw() {
			if(isVisible) {
				...
				if(buttonClose.event.clicked()) { setVisible(false); }
			}
		}
		...
		public final void setVisible(boolean isVisible) {
			...
			if(isVisible) {
				Window.this.layout.setPosition(x, y+bar.layout.getH());
				Window.this.layout.setSize(w, h-bar.layout.getH());
			} else {
